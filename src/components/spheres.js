...
function Boxes() {
  const colorDataFloat = useMemo(
    () =>
      Float32Array.from(
        new Array(amount)
          .fill()
          .flatMap((_, i) => tempColor.set(colorDataHex[i]).toArray())
      ),
    []
  )
  ...
  useFrame(state => {
    ...
    for (let i = 0; i < amount; i++) {
      tempObject.position.set(
        40 * Math.cos(time * moveSpeed + i * 0.5),
        40 * Math.sin(time * moveSpeed + i * 1.3),
        posZ[i]
      )
